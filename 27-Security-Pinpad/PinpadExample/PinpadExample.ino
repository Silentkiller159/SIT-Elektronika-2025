...
// Globalni promenne pro klavesnici
...
void setup() {
  ...
  keypadInit(); // Toto provede inicializaci maticove klavesnice
  lcdInit();    // Toto provede inicializaci displeje na I2C
}

/*
  Slouzi pro inicializaci LCD
*/
void lcdInit(){
  int error;
...
/*
  Slouzi pro inicializaci maticove klavesnice
*/
void keypadInit(){
  pinMode(LED_BUILTIN, OUTPUT);             // Heartbeat diode
...
void keypadLoop(){
    ...
    if(key == '*'){
      ...
      // Nesmime pridat!
      Serial.println("Zadano vice jak 4 znaky!");
    }
    else{
      text += key;
    }
